...
async function main() {
  ...
  const appBaseUrl = normalizeBaseUrl(
    required("APP_BASE_URL or --base-url", args["base-url"] ?? processRef.env.APP_BASE_URL)
  );
  const exportToken = required(
    "EXCEPTION_EXPORT_TOKEN or --token",
    args.token ?? processRef.env.EXCEPTION_EXPORT_TOKEN
  );
  ...
  const includePayload = args["include-payload"] === "0" ? "0" : "1";
  const owner = args.owner ?? "all";
  const caseStatus = args["case-status"] ?? "all";
  const view = args.view ?? "all";
  const sort = args.sort ?? "priority_desc";
  const overdueOnly = args["overdue-only"] === "1" ? "1" : "0";
  const outFile = withJsonExt(
    args.out ??
      `exception-export-${orgId}-${new Date().toISOString().slice(0, 19).replace(/[:T]/g, "-")}.json`
  );
  ...
  if (resumeFrom) {
    ...
    processRef.stdout.write(
      `[export] resumed rows=${resumeInfo.rows.length} from=${resumeInfo.absPath} next_offset=${offset}\n`
    );
  ...
  while (page < maxPages && (!resumeInfo || resumeInfo.hasMore || explicitOffset)) {
    ...
    const url = `${appBaseUrl}/api/operations/exceptions/export?${qp.toString()}`;
  ...
  const output = {
    meta: {
      ...
      requested: {
        org_id: orgId,
        limit,
        include_payload: includePayload === "1",
        owner,
        case_status: caseStatus,
        view,
        sort,
